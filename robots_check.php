...
if (isset($_POST['search'])) {
    ...
    for ($i=0; $i<$count_domains; $i++) {
        ...
        if (count($robot_list)!=count($visa_robots_ms)) {
            ...
            $resultForReport[$robots_url]='Не идентичны [error:page not available]';
        }
        else{
            ...
            if (count($tmp_res)!=0) {
                ...
                $resultForReport[$robots_url]='Не идентичны [error: http 302]';
            }
            else{
                $check_robots_result[$robots_url]='<span style=color:green>Идентичны</span>';
                $resultForReport[$robots_url]='Идентичны';
            }
        }
    }
    arsort($check_robots_result);
    arsort($resultForReport);
}
?>
...
<table width="100%" border="1" cellspacing="0" cellpadding="4">
    ...
    ?>
    <?php
    if (count($resultForReport)>0) {
        $f=fopen('files/cron_reports.txt', 'a+');
        fputs($f,'---------------------------------Robots check---------------------------------'."\n");
        foreach ($resultForReport as $key => $value) {
            $str=$key.' = '.$value."\n";
            fputs($f, $str);
        }
        fputs($f,'---------------------------------End of robots check---------------------------------');
    }
    ?>
    </tbody>
</table>

...
if (isset($_POST['search'])) {
    ...
    print_r($duplicates_desc);
    print_r($duplicates_title);die;
    $f=fopen('files/cron_report.txt', 'w+');
    fputs($f,'------------------------Duplicate metatags----------------------------');
    if(!empty($duplicates_title)){
        fputs($f,'Duplicate descriptions'."\t");
        fputs($f,'---------------------------------------------------------------------'."\t");
        ...
        fputs($f,'Duplicate titles not found'."\t");
    }
    fputs($f,'------------------------Duplicate metatags----------------------------');
    if(!empty($duplicates_title)){
        fputs($f,'Duplicate descriptions'."\t");
        ...
        fputs($f,'Duplicate descriptions not found'."\t");
    }
    fclose($f);
}
...
<table width="100%" border="1" cellspacing="0" cellpadding="4">
    ...
    <tbody>
    <?php
    if (isset($duplicates_desc)  or isset($duplicates_title)) { ?>
    <tr>
        <td>
            <?php
            if(empty($duplicates_desc)){
                $f=fopen('files/result.txt', 'w+');
                fputs($f,'Duplicates descriptions not found');
            }
            else{
                $f=fopen('files/result.txt', 'w+');
        ...
        <td>
            ...
            if(!empty($duplicates_desc)){
                $f=fopen('files/result.txt', 'w+');
                fputs($f,'Duplicates titles not found');
            }else{
